...
Right after install a Get Started page shows the three tiers with the model each one runs on and a **Choose tier models** link. In chat, on Auto or any model: the request, a **Composer 2.5 Fast** subagent card (the model is in the name), the answer. Risky work shows a **Claude Opus 5 Deep** card. The chat model writes no cost lines of its own. Savings are estimates from list prices with the delegation's whole cost counted (the subagent's session, the chat's dispatch and relay), calibrated on Cursor's bill: on Auto, Composer, Grok, Sonnet and GPT routine work stays in the chat because delegating it costs the same or more (measured on Auto: 6.4¢ in the chat, 7.5¢ delegated); routing pays from Opus-class chat models, and risky work goes to Deep from any model. A model your plan refuses is skipped for a few hours and the task steps down a tier, with a one-line note; a model refused repeatedly is replaced. **Choose tier models** in the status menu picks which model runs each tier, and the Settings UI (search "cost optimizer") holds every knob: tier models, enforce vs. advise, always delegate, budget per chat, cooldown hours, auto-enable.
...
- Per-call overhead: one hook process per delegation, edit or shell command (reads are not hooked): 60–110 ms with the compiled binary on Linux, about 650 ms on Windows where Cursor itself spends 330 ms spawning any hook (measured with a bare `echo`). Nothing is blocked by default.
