...
- **A chat of its own**: the `AI Cost Chat` view in the activity bar (or **Open cost-routed chat** from the status bar menu) is a chat panel the extension owns, so every reply is shown under the name of the model that produced it, with what it read, ran and changed (diffs inline, files one click away), what the turn cost from the token usage Cursor's CLI reports, and what the same tokens would have cost at Auto's billed rate. Each request is routed by the same scorer as the hooks (or forced to a tier from the picker) and runs on that tier's model through the Cursor CLI on your own account: no API keys, same bill. The cheap model does the work directly, with no chat paying to dispatch and relay it. Needs `cursor-agent` installed and logged in; the panel's structure follows Cline and Roo Code (see THIRD_PARTY_NOTICES.md).
