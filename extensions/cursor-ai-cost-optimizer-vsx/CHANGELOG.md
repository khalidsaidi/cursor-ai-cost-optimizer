...
- Remote windows (WSL, SSH, containers): Cursor's Task tool lists the subagents it knew when the window opened and never refreshes that list, so after an update, a re-map or a first setup made in that window every delegation failed with "Invalid enum value" (seen live in a WSL window: the model retried blindly). Old names are now kept as alias files on the new model until the next window opens, the hook turns the Task tool's own error into "retry with `<name this window accepts>`" (once), a window that knows none of the subagents keeps the work in the chat, and the status bar offers **Reload window** when a remote window needs it.
