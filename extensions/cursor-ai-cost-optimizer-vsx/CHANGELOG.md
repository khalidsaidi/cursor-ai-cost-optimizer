...
- The chat panel audited as a product against Copilot Chat and Cline, and rebuilt on their code where it existed: replies stream (the CLI's partial output, consolidated when it repeats a segment); the active file and selection travel with each request behind a switchable chip; conversations survive a reload and earlier ones reopen from History; a checkpoint of the workspace is taken before every turn with Roo Code's shadow-git checkpoint service (vendored) and restored in one click; edits render in Roo Code's DiffView and command output in Cline's CodeBlock inside Cline's CodeAccordian (vendored); a model at its usage limit on the plan is swapped for the next tier with one explanatory line and put on the hooks' six-hour cooldown; the model stays the same across a conversation unless a request needs a stronger tier (switching re-reads the whole context on the new model); Esc stops; a stopped turn says its edits are on disk. The extension host is now bundled with esbuild so the vendored service's dependencies ship.
