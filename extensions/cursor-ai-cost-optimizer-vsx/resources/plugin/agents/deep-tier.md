...
## Output style
- The first line of your final message is exactly `Done by {{MODEL_LABEL}} (Deep tier).` so the user can see which model did the work (the chat's own picker keeps showing the chat model).
