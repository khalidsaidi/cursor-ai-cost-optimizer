// Vendored from Cline (apps/vscode/webview-ui/src/components/common/CodeAccordian.tsx), Apache-2.0, see ../../LICENSE.
// One change: a diff renders with Roo Code's DiffView (line numbers, editor colours) instead of a ```diff code block.
		...
		<div className="bg-code overflow-hidden rounded-xs border border-editor-group-border">
			...
			{(!(path || isFeedback || isConsoleLogs) || isExpanded) && (
				<div className="overflow-x-auto overflow-y-hidden max-w-full">
					{diff !== undefined ? (
						<DiffView source={diff} filePath={path} />
					) : (
						<CodeBlock source={`${"```"}${inferredLanguage}\n${(code ?? "").trim()}\n${"```"}`} />
					)}
				</div>
			)}
		</div>
