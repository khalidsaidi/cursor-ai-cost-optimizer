...
test("everywhere scope: nothing in the repo, ~/.cursor hooks + agents, pause per project, remove leaves nothing", () => {
  ...
  try {
    ...
    const r = installUser(opts, stateRoot, ws);
    ...
    assert.ok(fs.existsSync(path.join(stateRoot, "plugin", "rules", "cco-routing.mdc")));
    ...
    assert.equal(pauseWorkspace(opts, stateRoot, ws, true).status, 0);
    assert.equal(workspacePaused(stateRoot, ws), true);
    assert.equal(pauseWorkspace(opts, stateRoot, ws, false).status, 0);
    assert.equal(workspacePaused(stateRoot, ws), false);
    // extension update: plugin path changes -> doctor re-runs the idempotent install
    const d = doctorUser({ ...opts, extensionVersion: "test2" }, stateRoot);
    assert.deepEqual(d, { installed: true, changed: true, actions: ["repointed_after_update"] });
    assert.equal(doctorUser({ ...opts, extensionVersion: "test2" }, stateRoot).changed, false);
    const un = uninstallUser(opts, stateRoot);
