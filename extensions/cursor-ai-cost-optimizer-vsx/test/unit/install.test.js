...
test("installWorkspace (node runtime): plugin layout + rule, nothing outside .cursor/, no skills/commands copies", async () => {
  ...
  try {
    ...
    for (const rel of ["hooks.json", "cco-hook.mjs", "cco/plugin-path.txt", "cco/runtime.json", "cco/extension-manifest.json", "agents/composer-2.5-fast.md", "cco/agent-names.json", "rules/cco-routing.mdc"]) {
    ...
    assert.match(hooks.preToolUse[0].matcher, /^\^\(Task\|Write/);
    assert.equal(r.agents["fast-tier"], "composer-2.5");
    assert.equal(r.agents["deep-tier"], "claude-opus-5-thinking-high");
    assert.ok(fs.readFileSync(path.join(p.agentsDir, "composer-2.5-fast.md"), "utf8").includes("generated by cursor-ai-cost-optimizer"), "named after the model it runs on");
    const status = workspaceStatus(ws);
    assert.equal(status.enabled, true);
    assert.equal(status.hookMode, "node");

    // idempotent; user-authored agent never overwritten
    fs.writeFileSync(path.join(p.agentsDir, "composer-2.5-fast.md"), "---\nname: composer-2.5-fast\nmodel: my-model\n---\nmine\n");
    await installWorkspace(ws, base());
    assert.equal(entriesOf(ws).sessionStart.length, 1);
    assert.match(fs.readFileSync(path.join(p.agentsDir, "composer-2.5-fast.md"), "utf8"), /model: my-model/);
    ...
    assert.equal(fs.existsSync(path.join(p.agentsDir, "claude-opus-5-deep.md")), false, "generated agent removed");
    assert.ok(fs.existsSync(path.join(p.agentsDir, "composer-2.5-fast.md")), "user-authored agent kept");
