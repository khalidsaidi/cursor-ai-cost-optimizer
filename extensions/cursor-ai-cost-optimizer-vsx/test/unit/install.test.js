...
test("installWorkspace (node runtime): plugin layout + rule, nothing outside .cursor/, no skills/commands copies", async () => {
  ...
  try {
    ...
    for (const rel of ["hooks.json", "cco-hook.mjs", "cco/plugin-path.txt", "cco/runtime.json", "cco/extension-manifest.json", "agents/fast-tier.md", "agents/tier-verifier.md", "rules/cco-routing.mdc"]) {
    ...
    assert.ok(fs.readFileSync(path.join(p.agentsDir, "fast-tier.md"), "utf8").includes("generated by cursor-ai-cost-optimizer"));
    const status = workspaceStatus(ws);
    assert.equal(status.enabled, true);
    assert.equal(status.hookMode, "node");

    // idempotent; user-authored agent never overwritten
    fs.writeFileSync(path.join(p.agentsDir, "fast-tier.md"), "---\nname: fast-tier\nmodel: my-model\n---\nmine\n");
    await installWorkspace(ws, base());
    assert.equal(entriesOf(ws).sessionStart.length, 1);
    assert.match(fs.readFileSync(path.join(p.agentsDir, "fast-tier.md"), "utf8"), /model: my-model/);
    ...
    assert.equal(fs.existsSync(path.join(p.agentsDir, "deep-tier.md")), false, "generated agent removed");
    assert.ok(fs.existsSync(path.join(p.agentsDir, "fast-tier.md")), "user-authored agent kept");
