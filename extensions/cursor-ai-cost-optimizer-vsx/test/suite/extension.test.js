...
describe('AI Cost Optimizer extension', () => {
  ...
  it('install writes only under .cursor/ (plugin layout + rule + hooks), preserving foreign hook entries', async () => {
    ...
    const files = ['.cursor/hooks.json', '.cursor/cco-hook.mjs', '.cursor/cco/plugin-path.txt', '.cursor/cco/runtime.json', '.cursor/cco/extension-manifest.json', '.cursor/cco/agent-names.json', '.cursor/rules/cco-routing.mdc'];
    ...
    const agentNames = JSON.parse(fs.readFileSync(path.join(root, '.cursor', 'cco', 'agent-names.json'), 'utf8'));
    assert.strictEqual(agentNames['fast-tier'], 'composer-2.5-fast', 'subagents are named after the model they run on');
    for (const name of ['fast-tier', 'balanced-tier', 'deep-tier', 'tier-verifier']) {
      const text = fs.readFileSync(path.join(root, '.cursor', 'agents', `${agentNames[name]}.md`), 'utf8');
  ...
  it('uninstall removes CCO pieces but keeps others', async () => {
    ...
    proof.uninstallChecks.agentsRemoved = !fs.existsSync(path.join(root, '.cursor', 'agents', 'composer-2.5-fast.md'));
    ...
    await vscode.commands.executeCommand('cco.installCursorAssets', { confirm: false, scope: 'project' });
    assert.ok(fs.existsSync(path.join(root, '.cursor', 'agents', 'composer-2.5-fast.md')));
  });
