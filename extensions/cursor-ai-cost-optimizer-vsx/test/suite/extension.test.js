...
describe('AI Cost Optimizer extension', () => {
  ...
  it('install writes only under .cursor/ (plugin layout + rule + hooks), preserving foreign hook entries', async () => {
    ...
    for (const name of ['cco-fast', 'cco-balanced', 'cco-deep', 'cco-verifier']) {
    ...
    assert.strictEqual(proof.installChecks.agents['cco-fast'], 'composer-2.5', 'tier model from (fake) discovery');
