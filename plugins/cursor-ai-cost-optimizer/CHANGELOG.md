...
### Changed
- Subagents renamed for the user (Cursor shows the name on the card): `fast-tier`, `balanced-tier`, `deep-tier`, `fast-research`, `tier-verifier`. `cco-*` names still route; generated `cco-*.md` files are replaced on init/update and removed on uninstall.
