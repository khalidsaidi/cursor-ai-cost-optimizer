...
import { analyzeOutcome, parseEnumError } from "../scripts/cco-task-result.mjs";
...
import { workspacePaths, agentForTier } from "../scripts/lib/common.mjs";
import { writeWorkspaceAgents, readWorkspaceAgentModel, GENERATED_MARKER, removeAliasAgents } from "../scripts/lib/agents.mjs";
...
test("agents generated by an older CCO version are refreshed on upgrade; user-authored ones are kept", async () => {
  ...
  const alias = fs.readFileSync(path.join(dir, "fast-tier.md"), "utf8");
  assert.match(alias, /cco-alias-for: composer-2\.5-fast/, "old generated name kept as an alias (this window still lists it)");
  assert.match(alias, /model: old-model/, "on the model the window loaded it with: what still runs there");
  removeAliasAgents(ws);
  assert.equal(fs.existsSync(path.join(dir, "fast-tier.md")), false, "aliases go when a window opens");
...
test("legacy cco-* names: old rules still route, and generated cco-*.md files are replaced on setup", async () => {
  ...
  assert.match(fs.readFileSync(path.join(dir, "cco-fast.md"), "utf8"), /cco-alias-for: composer-2\.5-fast/, "generated legacy agent kept as an alias until the next window");
...
test("a window that still lists the old subagent names: the Task tool's error is turned into the alias to use, and the context names it", () => {
  const ws = readyWorkspace();
  const parsed = parseEnumError("Invalid arguments:\nsubagent_type: Invalid enum value. Expected 'generalPurpose' | 'explore' | 'cco-fast' | 'cco-deep', received 'composer-2.5-fast'");
  assert.deepEqual(parsed, { accepted: ["generalPurpose", "explore", "cco-fast", "cco-deep"], received: "composer-2.5-fast" });
  // the old-name file is an alias on the new model (as writeWorkspaceAgents leaves it after an upgrade)
  const dir = workspacePaths(ws).agentsDir;
  fs.writeFileSync(path.join(dir, "cco-fast.md"), `---\nname: cco-fast\nmodel: composer-2.5\n---\n${GENERATED_MARKER}\n<!-- cco-alias-for: composer-2.5-fast -->\nbody\n`);
  const out = runHook("cco-task-result.mjs", { hook_event_name: "subagentStop", conversation_id: "w1", subagent_type: "unknown", status: "error", message_count: 0, tool_call_count: 0, error_message: "Invalid arguments:\nsubagent_type: Invalid enum value. Expected 'generalPurpose' | 'cco-fast' | 'cco-deep', received 'composer-2.5-fast'", workspace_roots: [ws] });
  assert.match(out.additional_context, /subagent_type="cco-fast"/, "the model is told the name this window accepts");
  assert.equal(agentForTier("fast", ws), "cco-fast", "names handed out now follow the window");
  assert.equal(fs.existsSync(workspacePaths(ws).limitsPath), false, "not a usage limit: no model cooldown");
  const ctx = runHook("cco-prompt-capture.mjs", { hook_event_name: "beforeSubmitPrompt", conversation_id: "w2", prompt: "add a helper", workspace_roots: [ws] });
  assert.match(ctx.additional_context, /`cco-fast`/);
  // a window that lists none of ours (set up after it opened): work stays in the chat, quietly
  const none = runHook("cco-task-result.mjs", { hook_event_name: "subagentStop", conversation_id: "w1", subagent_type: "unknown", status: "error", message_count: 0, tool_call_count: 0, error_message: "Invalid arguments:\nsubagent_type: Invalid enum value. Expected 'generalPurpose' | 'explore', received 'composer-2.5-fast'", workspace_roots: [ws] });
  assert.match(none.additional_context, /Do this task in this chat/);
  const gate = runHook("cco-tool-gate.mjs", { hook_event_name: "preToolUse", conversation_id: "w1", tool_name: "Write", tool_input: { file_path: "a.js", content: "x" }, workspace_roots: [ws] });
  assert.equal(gate.permission, "allow");
  assert.equal(gate.agent_message, undefined, "no delegation advice when there is nothing to delegate to");
  const briefing = runHook("cco-prompt-capture.mjs", { hook_event_name: "beforeSubmitPrompt", conversation_id: "w3", prompt: "add a helper", workspace_roots: [ws] });
  assert.match(briefing.additional_context, /after the window is reloaded/);
  // a new window: workspaceOpen drops the aliases and records the current names
  runHook("cco-workspace-open.mjs", { hook_event_name: "workspaceOpen", workspace_roots: [ws] });
  assert.equal(fs.existsSync(path.join(dir, "cco-fast.md")), false);
  assert.equal(agentForTier("fast", ws), "composer-2.5-fast");
