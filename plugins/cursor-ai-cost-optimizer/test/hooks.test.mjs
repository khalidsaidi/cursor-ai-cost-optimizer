...
test("agents generated by an older CCO version are refreshed on upgrade; user-authored ones are kept", async () => {
  ...
  assert.match(fs.readFileSync(path.join(dir, "fast-tier.md"), "utf8"), /cco-alias-for: composer-2\.5-fast[\s\S]*model: composer-2\.5|model: composer-2\.5[\s\S]*cco-alias-for: composer-2\.5-fast/, "old generated name kept as an alias on the new model (this window still lists it)");
